...
def camera_auto_exposure_algo(
    ...
    # shutter/analog gain registers will be updated after function returns
    ...
    green_gain_uint16 = int(last_green_gain * 256.0)
    blue_gain_uint16 = int(last_blue_gain * 256.0)

    # Safety check for per-channel gain overflow
...
async def main():
    """
    Implements the auto exposure and white balance algorithm from the Frame firmware v25.031.0924
    """
    frame = FrameMsg()
    try:
        await frame.connect()
        ...
        for i in range(10):
            ...
            tx_manual_exp = TxManualExpSettings(manual_shutter=int(algo_result['shutter']),
                                                ...
                                                manual_red_gain=int(algo_result['red_gain']*256.0),
                                                manual_green_gain=int(algo_result['green_gain']*256.0),
                                                manual_blue_gain=int(algo_result['blue_gain']*256.0),
                                                )
